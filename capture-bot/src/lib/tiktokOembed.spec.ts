...
describe('formatTikTokNote', () => {
  it('includes caption, creator link, and original URL', () => {
    ...
    expect(body).toContain('**Creator:** [Chef Ana](https://www.tiktok.com/@chef)');
    expect(body).toContain('**Original URL:** https://www.tiktok.com/@chef/video/1');
  });
});
...
describe('fetchTikTokOembed', () => {
  ...
  it('returns parsed oEmbed fields on success', async () => {
    ...
    const result = await fetchTikTokOembed('https://www.tiktok.com/@mover/video/99');
  ...
  it('returns null on non-2xx responses', async () => {
    ...
    await expect(fetchTikTokOembed('https://vm.tiktok.com/bad/')).resolves.toBeNull();
  });
  ...
  it('returns null on network errors and timeouts', async () => {
    ...
    await expect(fetchTikTokOembed('https://vm.tiktok.com/slow/')).resolves.toBeNull();
  });
});
