...
const FASTER_WHISPER_SCRIPT = path.join(
  __dirname,
  '..',
  '..',
  'scripts',
...
async function transcribeWithWhisperCpp(
  ...
  console.log(
    `${LOG_PREFIX} whisper.cpp: ${config.whisperCppBin} ${args.join(' ')}`,
  );
  ...
  try {
    ...
    if (transcript) {
      console.log(
        `${LOG_PREFIX} whisper.cpp: preview="${transcript.slice(0, 120)}${transcript.length > 120 ? '…' : ''}"`,
      );
    }
...
async function transcribeWithFasterWhisper(
  ...
  console.log(
    `${LOG_PREFIX} faster-whisper: ${config.fasterWhisperPython} ${args.join(' ')}`,
  );

  try {
    const { stdout, stderr } = await execFileAsync(
      config.fasterWhisperPython!,
      args,
      {
        timeout: TRANSCRIBE_TIMEOUT_MS,
        encoding: 'utf-8',
    ...
    const transcript = stdout.trim();
    console.log(
      `${LOG_PREFIX} faster-whisper: transcript length=${transcript.length} chars`,
    );
    return transcript;
  } catch (err) {
    const error = err as Error & { stderr?: string; stdout?: string };
